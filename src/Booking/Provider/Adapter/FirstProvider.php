...
use App\Client\First\FirstClient;
...
class FirstProvider implements ProviderInterface
{
    ...
    public function __construct(string $name, FirstClient $client, LoggerInterface $logger)
    {
        $this->name   = $name;
        $this->client = $client;
        $this->logger = $logger;
    ...
    {
        $offers = $this->client->findOffers($this->transformParams($params));
    ...
    {
        $response = $this->client->book($offerId);
